...
2. 应用场景：比如当前表建了一个联合索引(a, b, c)，使用`where`条件的时候，由于b用得是 `like '%xxx%'` 需要回表查询 (`like 'xx%'` 不需要)。即先查询 `a = '1' `的数据， 然后回表查询，最后进行`where`条件的过滤。如果使用索引下推之后 (MySQL 5.6)，在查完`a = '1'`的数据之后，可以先由存储引擎层进行`where`条件过滤，然后再回表查询， 减少回表查询的次数。
