...
3. 执行`hincrby`，设置重入计数为`1` （可重入锁才有这一步操作）
4. 执行`pexpire`， 设置锁的过期时间 （为了防止任务还没执行完，锁就过期了。Redisson实现了用看门狗机制来为锁进行自动续期）
...
热点`key` 问题就是某个瞬间，大量的请求集中访问Redis里的同一个固定`key`，假如热点`key`过期，可能会导致缓存击穿，让大量的请求直接打到数据库里面。像`热点新闻`、`热点评论`、`明星直播` 这种读多写少的场景，就很容易出现热点`key` 问题。因为Redis的单节点查询性能一般在 `2w QPS`， 一般超过 这个数值，可能就会宕机。
